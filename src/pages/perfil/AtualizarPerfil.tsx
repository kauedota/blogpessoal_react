...
function AtualizarPerfil() {
    ...
    setIsLoading(true);
			...
			try {
				...
				ToastAlerta("Usuário atualizado com sucesso! Efetue o Login Novamente!", "sucesso")
				sucesso()
			} catch (error) {
...
<p className="text-white/80 text-base mt-2">{user.usuario}</p>
</div>
...
<input
										...
										name="nome"
										placeholder="Nome"
										className="px-4 py-2 border-2 border-line rounded focus:outline-none"
										value={user.nome || ""}
...
<input
										...
										name="usuario"
										placeholder="Usuario"
										className="px-4 py-2 border-2 border-line rounded bg-cream-dark cursor-not-allowed"
										disabled
										value={user.usuario || ""}
										onChange={(e: ChangeEvent<HTMLInputElement>) =>
											atualizarEstado(e)
...
<button
										type="button"
										className="rounded text-white bg-danger hover:bg-danger-dark w-1/2 py-2 font-bold"
										onClick={retornar}
>
										Cancelar
</button>
									...
									<button
										type="submit"
										className="rounded text-white bg-primary hover:bg-primary-dark
